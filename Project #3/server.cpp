...
void Create_Chatroom(int newConnection, char* buffer){
    int random_number = buffer[3]-'0';
    string tmp_port="";
    for(int i=5;i<strlen(buffer);i++){
        if(buffer[i]=='\0'){
    ...
    string send_back="";
    if(Chat_Status[random_number]=="UNDEFINED"){
        Chat_Status[random_number]="OPEN";
        Chat_port[random_number]=port;
        send_back="S";
    }
    else if(Chat_Status[random_number]=="UNDEFINED"){
        send_back="N";
    }
    else{
        send_back="R";
...
void List_Chatroom(int UDP_socket, struct sockaddr_in &client_info){
    string send_back = "Chatroom_name\tStatus";
    for(int i=0;i<Chat_Status.size();i++){
        if(Chat_Status[i]=="UNDEFINED"){
            continue;
        }
        else{
...
void Join_Chatroom(int newConnection, char* buffer){
    string tmp="";
    for(int i=3;i<strlen(buffer);i++){
        if(buffer[i]=='\0'){
            break;
    ...
    for(int i=0;i<ClientTable.size();i++){
        if(ClientTable[i] == tmp){
            if(Chat_Status[i]=="OPEN"){
                send_back=""+to_string(Chat_port[i]);
            }
            else{
                send_back="F";
            }
...
void Restart_Chatroom(int newConnection, char* buffer){
    int random_number = buffer[3]-'0';
    string send_back="";
    if(Chat_Status[random_number]=="CLOSED"){
        send_back="S";
        Chat_Status[random_number]="OPEN";
    }
    else if(Chat_Status[random_number]=="UNDEFINED"){
...
void Leave_Chatroom(int newConnection, char* buffer){
    int random_number = buffer[3]-'0';
    Chat_Status[random_number]="CLOSED";
}
void *TCP_connection(void *parameter){
    ...
    while(1){
        ...
        switch(buffer[0]){
            ...
            case '7':
                switch(buffer[1]){
                    // Create_Chatroom command
                    case '0': Create_Chatroom(newConnection, buffer); break;
                    // Join_Chatroom command
                    case '2': Join_Chatroom(newConnection, buffer); break;
                    // Restart_Chatroom command
                    case '3': Restart_Chatroom(newConnection, buffer); break;
                    // Leave_Chatroom command
                    case '4': Leave_Chatroom(newConnection, buffer);  break;
                    default: break;
                }
                break;
            default: break;
...
int main(int argc, char* argv[]){
    ...
    while(1){
        ...
        if (FD_ISSET(UDP_socket, &rset)){
            ...
            switch(buffer[0]){
                // Register command
                case '0': Register(UDP_socket, buffer, client_info, database);    break;
                // Whoami command
                case '3': Whoami(UDP_socket, buffer, client_info);    break;
                case '7': List_Chatroom(UDP_socket, client_info); break;
                default:    break;
            }
            // Flush the buffer
            memset(&buffer, '\0', sizeof(buffer));
